...
public class Validator {
	...
	public static int getInt(Scanner scnr, String prompt, int min, int max) {
		...
		int number;
		do {
			number = getInt(scnr, prompt); // not recursion, using getInt method above
	...
	public static double getDouble(Scanner scnr, String prompt, double min, double max) {
		...
		do {
			...
			if (number < min) { // checking if it is less than the min, will include that min number you set
				isValid = false;
				System.out.println("The number must be at least " + min);
			} else if (number > max) {
				isValid = false;
